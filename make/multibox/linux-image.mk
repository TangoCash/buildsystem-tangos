...
flashimage: $(D)/neutrino
	$(START_BUILD)
	$(MAKE) flash-image-multibox-multi-disk flash-image-multibox-multi-rootfs
	$(TUXBOX_CUSTOMIZE)
	@$(call draw_line,);
...
ofgimage: $(D)/neutrino
	$(START_BUILD)
	$(MAKE) ITYPE=ofg flash-image-multibox-multi-rootfs
	$(TUXBOX_CUSTOMIZE)
	@$(call draw_line,);
	@$(call draw_line,Build of $@ for $(BOXTYPE) successfully completed.,2);
	@$(call draw_line,);
...
online-image: $(D)/neutrino
	$(START_BUILD)
	$(MAKE) ITYPE=online flash-image-multibox-online
	$(TUXBOX_CUSTOMIZE)
	@$(call draw_line,);
	@$(call draw_line,Build of $@ for $(BOXTYPE) successfully completed.,2);
	@$(call draw_line,);
...
FLASH_BOOTARGS_SRC = multibox-bootargs-$(FLASH_BOOTARGS_DATE).zip
FLASH_PARTITONS_DATE = 20200319
FLASH_PARTITONS_SRC = multibox-partitions-$(FLASH_PARTITONS_DATE).zip
FLASH_RECOVERY_DATE = 20210118
FLASH_RECOVERY_SRC = multibox-recovery-$(FLASH_RECOVERY_DATE).zip
...
flash-image-multibox-multi-disk: $(ARCHIVE)/$(FLASH_BOOTARGS_SRC) $(ARCHIVE)/$(FLASH_PARTITONS_SRC) $(ARCHIVE)/$(FLASH_RECOVERY_SRC)
...
flash-image-multibox-multi-rootfs:
	rm -rf $(FLASH_BUILD_TMP) || true
	mkdir -p $(FLASH_BUILD_TMP)/$(BOXTYPE)
	cp $(RELEASE_DIR)/boot/uImage $(FLASH_BUILD_TMP)/$(BOXTYPE)/uImage
	cd $(RELEASE_DIR); \
