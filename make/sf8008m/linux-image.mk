...
flashimage: $(D)/neutrino
	$(START_BUILD)
	$(MAKE) flash-image-sf8008m-multi-disk flash-image-sf8008m-multi-rootfs
	$(TUXBOX_CUSTOMIZE)
	@$(call draw_line,);
...
ofgimage: $(D)/neutrino
	$(START_BUILD)
	$(MAKE) ITYPE=ofg flash-image-sf8008m-multi-rootfs
	$(TUXBOX_CUSTOMIZE)
	@$(call draw_line,);
	@$(call draw_line,Build of $@ for $(BOXTYPE) successfully completed.,2);
	@$(call draw_line,);
...
online-image: $(D)/neutrino
	$(START_BUILD)
	$(MAKE) ITYPE=online flash-image-sf8008m-online
	$(TUXBOX_CUSTOMIZE)
	@$(call draw_line,);
	@$(call draw_line,Build of $@ for $(BOXTYPE) successfully completed.,2);
	@$(call draw_line,);
...
FLASH_PARTITONS_SRC = $(MACHINE)-partitions-$(FLASH_PARTITONS_DATE).zip
...
flash-image-sf8008m-multi-disk: $(ARCHIVE)/$(BUILDIMAGE_SRC) $(ARCHIVE)/$(FLASH_PARTITONS_SRC)
...
flash-image-sf8008m-multi-rootfs:
	rm -rf $(FLASH_BUILD_TMP) || true
	mkdir -p $(FLASH_BUILD_TMP)/$(BOXTYPE)
	cp $(RELEASE_DIR)/tmp/uImage $(FLASH_BUILD_TMP)/$(BOXTYPE)/uImage
	cd $(RELEASE_DIR); \
