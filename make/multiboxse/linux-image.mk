...
FLASH_BOOTARGS_SRC = multiboxse-bootargs-$(FLASH_BOOTARGS_DATE).zip
FLASH_PARTITONS_DATE = 20200319
FLASH_PARTITONS_SRC = multiboxse-partitions-$(FLASH_PARTITONS_DATE).zip
FLASH_RECOVERY_DATE = 20201110
FLASH_RECOVERY_SRC = multiboxse-recovery-$(FLASH_RECOVERY_DATE).zip
...
flash-image-multiboxse-multi-disk: $(ARCHIVE)/$(FLASH_BOOTARGS_SRC) $(ARCHIVE)/$(FLASH_PARTITONS_SRC) $(ARCHIVE)/$(FLASH_RECOVERY_SRC)
...
flash-image-multiboxse-multi-rootfs:
	rm -rf $(FLASH_BUILD_TMP) || true
	mkdir -p $(FLASH_BUILD_TMP)/$(BOXTYPE)
	cp $(RELEASE_DIR)/boot/uImage $(FLASH_BUILD_TMP)/$(BOXTYPE)/uImage
	cd $(RELEASE_DIR); \
